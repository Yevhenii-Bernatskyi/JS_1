...
const results = (a, b, c, alpha, beta) =>{
    ...
    if (alpha <= 0 || alpha >= 90 || beta <= 0 || beta >= 90) {
        console.log("Некоректні значення кутів. Перевірте введені дані.");
        return "failed";
    }

     if (alpha.toFixed(2) == 90 || beta.toFixed(2) == 90 || alpha.toFixed(2)==0 || beta.toFixed(2)==0) {
